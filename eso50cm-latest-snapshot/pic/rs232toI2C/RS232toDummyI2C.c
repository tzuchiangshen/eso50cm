...
int write_i2c_data( char * m_buffer ) {
    ...
    i2c_start();
    ack  = i2c_write( m_buffer[0] );
    ack += i2c_write( m_buffer[1] );
    for( i = 2; i < 6; i ++ ) {
        ack += i2c_write( m_buffer[i] );
    }
    i2c_stop();
    */
    return ack;
    ...
    switch( buffer[1] ) {
        ...
        case 'R':
            ...
            do {
                k += 2;
                i = 0;
                if( buffer[k] == '2' || buffer[k] == '4' || buffer[k] == '6'|| buffer[k] == '8' ) {
                    i = buffer[k] - '0';   
                    i /= 2;
                } else if( buffer[k] == 'A' || buffer[k] == 'C'  ) {
                    i = buffer[2] - 'A';   
                    i += 10;
                    i /= 2;
                }
                if( 0 < i && i < NUM_I2C_DEV ) {
                    ...
                    if( i2c_memory < 10 ) {
                        ...
                        printf( ", Wrong mem address %c#", i2c_memory );
                    }
                } else {
                    printf( ":Wrong device %c#", buffer[k]  );     
                }    
            } while( buffer[k+2] != '#' );
            break;
        case 'S':
            if( buffer[2] == '#' ) {
                if( scan_all_encoders ) {
                    printf( ":Stop scanning all encoders#"  );
                    scan_all_encoders = false;
                } else {
                    printf( ":Scanning all encoders#"  );
...
void binaryWriteMessage( char * buffer ) {
	...
	i2c_buffer[0] = buffer[1];
	i2c_buffer[1] = buffer[2];
	for( i = 1; (i < NUM_I2C_DEV) && (i2c_device[i] != i2c_buffer[0]); i ++ );
    if( i <  NUM_I2C_DEV ) {
        //printf( ":Address 0x%2X detected#", i2c_device[i] );
...
void binaryTestMessage( char * buffer ) {
	...
	i2c_buffer[0] = 0xFE & buffer[1];
	for( i = 1; (i < NUM_I2C_DEV) && (i2c_device[i] != i2c_buffer[0]); i ++ );
    if( i <  NUM_I2C_DEV ) {
        i2c_status[i] = test_i2c_device( i2c_buffer[0] );
        putc( ':' );
        putc( i2c_buffer[0] );
        putc( i2c_status[i] );
        putc( '#' ); 
...
void binaryMessage( char * buffer ) {
    ...
    //putc( ':' );
    do{
	    for( i = 1; (i < NUM_I2C_DEV) && (i2c_device[i] != (0xFE & buffer[j])); i ++ );
        if( i <  NUM_I2C_DEV ) {
