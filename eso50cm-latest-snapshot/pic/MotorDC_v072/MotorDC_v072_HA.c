...
#define version 702
#pragma id version
#define __PRODUCT__ "MotorDC_v071"
#undef DEBUG
//#define DEBUG
...
signed int32    encRev;
signed int32    encPos;
signed int32    encPosOld;
signed int32    encVel;
signed int32    encVelOld;
...
void localMessage( char * buffer ) 
{
    ...
    switch( buffer[1] ) {
        ...
        */  
        case 's':
            sprintf( ans_buffer, "speed = %f", avg );
            avg = 0;
            n_avg = 1;  
            break;
        case 'P':
        ...
        if( tmr0_XHz_flag ) {   // tmr0_XHz Routine
            ...
            * ( ( (char *) & POSCNT ) )     = PIC_POSCNTL;
            ...
            if( goto_flag == 1 ) {
	            ...
	            // With QEI at 2x 
	            // o In RA
	            //   144000 tics = 1 degree
	            //    36000 tics = 1 arcmin
	            //      600 tics = 15 arcsec
	            //       40 tics = 1 arcsec
	            // o In Dec
	            //   144000 tics = 1.25 degrees
	            //   115200 tics = 1 degree
	            //      480 tics = 15 arcsec
	            //       32 tics = 1 arcsec
                ...
                if( diffPos < 38400 ) {                 // < 8[arcmin]
                    ...
                    if( diffPos < 20 )  {
                        ...
                        printf( ":end goto#" );
                        #endif
                	} else if( diffPos < 2400 ) {       // < 30 arcsec
                    	trgEncVelRef = 600;             // 0.125[arcmin/s]
                    	goto_flag = 2;
                  	} else if( diffPos < 38400 ) {      // < 1.66 arcmin
                    	trgEncVelRef  = diffPos;
                    	trgEncVelRef *= 0.52;
                    	trgEncVelRef += 600;
                    	goto_flag = 2;
                  	} else {
                      	trgEncVelRef  = 38400;          // 8[arcmin/sec]
                      	goto_flag = 2;
                    }
                    sec = p_tmr0rv;
                } else { //if( diffPos >= 38400 ) 
              	    if( diffPos < 76800 ) {             // < 16[arcmin]
              	        trgEncVelRef = 19200;           // 4[arcmin/s]
              	        trgThreshold = 19200;
              	    } else if( diffPos < 153600 ) {     // < 32[arcmin]
              	        trgEncVelRef = 38400;           // 8[arcmin/s]
              	        trgThreshold = 38400;
              	    } else if( diffPos < 307200 ) {     // < 64[arcmin]
              	        trgEncVelRef = 76800;           // 16[arcmin/s]
              	        trgThreshold = 76800;
              	    } else if( diffPos < 614400 ) {     // < 128[arcmin]
              	        trgEncVelRef = 153600;          // 32[arcmin/s]
              	        trgThreshold = 153600;
              	    } else {
              	        trgEncVelRef = 153600;          // 32[arcmin/s]
              	        trgThreshold = 153600;
                ...
                m_MtrCtrl = NEWPID;
      	        #ifdef DEBUG
      	        printf( ":%8ld %8ld %8ld %8.0f %4ld %d#", encPos, trgPos, diffPos, trgEncVelRef, m_V, m_dir );
                #endif
            } else if( goto_flag == 2 ) {
                ...
                #ifdef DEBUG
                if( sec ==  0 ) {
                    printf( ":%8ld %8ld %8ld %4ld %d#", encPos, trgPos, diffPos, m_V, m_dir, );
                    sec = p_tmr0rv;
                } else {
                    sec --;
                }
            ...
            switch( m_MtrCtrl ) {
                ...
                case NEWPID:
                    ...
                    if( tmp_float == 0 ){
                        ...
                        if( -20400 <  encVelRef && encVelRef < 20400 ) {
                            h_pid_flag = false;
                        } else {
                            h_pid_flag = true;
                        }
                        if( encVel != 0 ) {
                            if( encVelRef > encVelRef_old ) {
                                if( encVelRef - encVelRef_old > 9600 ) {
                                    if( h_pid_flag ) {
                                        m_MtrCtrl = STOPSLOWLY; 
                                    } else {
                                        m_MtrCtrl = STOP;
                                ...
                                if( encVelRef_old - encVelRef > 9600 ) {
                                    if( h_pid_flag ) {
                                        m_MtrCtrl = STOPSLOWLY; 
                                    } else {
                                        m_MtrCtrl = STOP;
