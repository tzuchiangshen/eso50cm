...
/**
 * HTTP response, header, and body, indicating that we didnt
...
static void handle_get( int connection_fd, const char * page ) {
    ...
    if( * page == '/' && strchr (page + 1, '/' ) == NULL) {
        * arguments = 0;
        if( (ptr = strchr( page, '?' )) != NULL) {
            /** The has arguments, then extracts them */
    ...
    if (module == NULL) {
        /** Either the requested page was malformed, or we couldnt open a
...
void server_run (struct in_addr local_address, uint16_t port) {
    ...
    if (verbose) {
        ...
        /** Find the sockets local address.  */
    ...
    while (1) {
        ...
        child_pid = fork ();
        if (child_pid == 0) {
            /** This is the child process.  It shouldnt use stdin or stdout,
                so close them.  */
            if( ! verbose ) {
                close (STDIN_FILENO);
                close (STDOUT_FILENO);
            }
            /** Also this child process shouldnt do anything with the
                listening socket.  */
            close (server_socket);
