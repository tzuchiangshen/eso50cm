...
extern "C" void module_generate( int fd, const char * arguments, myLCU * lcu )
{
    ...
    if( verbose ) printf( "[stop_slewing] Hello world!\n" );
    if( * arguments != 0 ) {
        if( verbose ) printf( "[stop_slewing] has arguments %s\n", arguments );
        char * ptr = NULL;

        if( (ptr = strstr( arguments, "ra=" )) != NULL ) {
            if( sscanf( ptr, "ra=%d:%d:%d", & trg_ra_hrs, & trg_ra_min, & trg_ra_sec ) == 3 ) {
                if( verbose ) printf( "[stop_slewing] trg ra =%+03d:%02d:%02d\n", trg_ra_hrs, trg_ra_min, trg_ra_sec );
                ...
                if( verbose ) printf( "[stop_slewing] trg ra =%lf\n", trg_ra );
            } else {
                if( verbose ) printf( "[stop_slewing] No target ra data\n" );
            }
        }
        if( (ptr = strstr( arguments, "dec=" )) != NULL ) {
            if( sscanf( ptr, "dec=%d:%d:%d", & trg_dec_deg, & trg_dec_min, & trg_dec_sec ) == 3 ) {
                if( verbose ) printf( "[stop_slewing] trg dec=%+03d:%02d:%02d\n", trg_dec_deg, trg_dec_min, trg_dec_sec );
                ...
                if( verbose ) printf( "[stop_slewing] trg dec=%lf\n", trg_dec );
            } else {
                if( verbose ) printf( "[stop_slewing] No target dec data\n" );
            }
        }
    }
    ...
    if( verbose) printf( "[stop_slewing] My file descriptor is %d\n", fd );
    ...
    if( verbose) printf( "[stop_slewing] Good bye!\n" );
}
    ...
    if( * arguments != 0 ) {
        ...
        if( (ptr = strstr( arguments, "rate=" )) != NULL ) {
            if( sscanf( ptr, "rate=%c", & slew_rate ) == 1 ) {
                if( verbose ) printf( "[stop_slewing] slew_rate = %c\n", slew_rate );
            }
        }
        if( (ptr = strstr( arguments, "dir=" )) != NULL ) {
            if( sscanf( ptr, "dir=%c", & slew_dir ) == 1 ) {
                if( verbose ) printf( "[stop_slewing] slew_dir = %c\n", slew_dir );
            }
        }
    ...
    if( is_telescope_configured == 0 ) {
        ...
        lcu->waitSemaphore();
        {
            ...
            if( slew_dir == 'n' ) {
                ...
                lcu->telescope->alpha->Motor->setDeviceMemory( 7, & tics, 0  );
                lcu->telescope->alpha->Motor->setDeviceMemory( 6, & tics_per_sec, 0  );

                lcu->telescope->delta->Motor->setDeviceMemory( 7, & tics, 0  );
                lcu->telescope->delta->Motor->setDeviceMemory( 6, & tics_per_sec, 0  );
            }
