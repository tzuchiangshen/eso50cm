...
static char * format_info( const char * description, const char * data ) {
    ...
    result_length = strlen( description ) + strlen( data ) + 128;
...
void module_generate (int fd) {
    ...
    if( semaphore_id > 0 &&  segment_id > 0 ) {
        ...
        gmtime_r( & (telescope->gtime.tv_sec), & UTime  );
        ...
        timeinfo = format_info( "Universal Date", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        vec[vec_length].iov_base = timeinfo;
        vec[vec_length].iov_len = strlen( timeinfo );
        ++vec_length;

        strftime( line, 24, "%T", & UTime );
        timeinfo = format_info( "Universal Time", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        ...
        timeinfo = format_info( "Loca Date", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        ...
        timeinfo = format_info( "Local Time", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        ...
        JDate = julianDate( UTime );
        sprintf( line, "%20.6lf", JDate );
        timeinfo = format_info( "Julian Date", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        ...
        timeinfo = format_info( "Local Sidereal Time", line );
        if( vec_length == vec_size - 1 ) {
            vec_size *= 2;
            vec = xrealloc (vec, vec_size * sizeof (struct iovec));
        }
        ...
        timeinfo = "<tr><td colspan=\"3\" align=\"center\">Unable to Comply!</td></tr>";
