...
extern "C" void module_generate( int fd, const char * arguments, myLCU * lcu )
{
    ...
    if( verbose ) printf( "[goto_target] Hello world!\n" );
    /** Checks for arguments */
    if( * arguments != 0 ) {
        if( verbose ) printf( "[goto_target] Has arguments: %s\n", arguments );
        char * ptr = NULL;

        if( (ptr = strstr( arguments, "txt" )) != NULL ) {
            html_output_flag = 0;
            if( verbose ) printf( "[goto_target] No HTML output [%s]\n", ptr );
        }
    }
    ...
    if( html_output_flag ) {
        buf_len += sprintf( buffer, page_start, "Goto Target (goto_target)" );
    } else {
        buf_len += sprintf( buffer, "Goto Target (goto_target)" );
    }
    ...
    if( retval > 0 ) {
        if( verbose ) printf( "[goto_target] %d Bytes written in fd=%d \n", retval, fd );
    } else {
        perror( "[goto_target] write" );
        if( verbose ) printf( "[goto_target] write ERROR\n" );
    }
